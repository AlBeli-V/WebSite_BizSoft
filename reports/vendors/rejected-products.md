...
| Lansweeper | Вся линейка (Starter / Pro / Enterprise) | Проверка 02.09.2026 не подтвердила публичную цену и self-service checkout: страница тарифов закрыта проверкой Cloudflare — `ops-probe` получил HTTP 403 (`cf-mitigated: challenge`), `ops-price-scan` с рендером Chrome вернул пустой результат. По правилу допуска §2 без подтверждённого чекаута вендор не заводится. Спрос по замеру 02.09.2026 — 326 показов в месяц, то есть цена вопроса невелика; возобновлять только после ручной проверки оператором с браузера, включая готовность вендора оформить лицензию на российское юрлицо. | https://www.lansweeper.com/pricing/ |
