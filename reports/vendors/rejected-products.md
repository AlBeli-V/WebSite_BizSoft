# Не заведённые продукты/тарифы (после сверки прайса 2026-08-19)

Вендоры Slack, Cloudflare и 1Password сняты с витрины целиком по решению руководителя 19.08.2026 (карточки архивированы, лендинги убраны).

**29.08.2026: Slack возвращён на витрину** решением руководителя по данным спроса Вордстата (123 запроса в месяц; проверка 32 зарубежных разработчиков вне каталога). Карточки SLACK-PRO и SLACK-BUSINESS-PLUS перезаведены по свежим ценам вендора (`scripts/catalog/slack.json`). Cloudflare и 1Password остаются снятыми.

## Отклонённые вендоры (решение руководителя, повторно не предлагать)

**Отказ действует, только если он записан в машинный реестр.** Этот файл —
пояснение для человека, а не источник для системы. Отказ вносится в
`reports/seo/wordstat/decisions.json` (раздел `rejected`) и в
`reports/seo/intelligence/vendor-decisions.json`; оба файла читают и
исследование (`scripts/seo/wordstat/audience.py`), и письмо с веб-отчётом
(`scripts/seo/report_v4.py`), так что запись в любой из них закрывает вопрос —
но заводим в оба, чтобы формат каждого оставался полным. Хранилище — ветка
`seo-data`, синхронизация `scripts/seo/data_sync.sh`.

| Вендор | Причина | Решение | В машинном реестре |
|---|---|---|---|
| NordVPN (и любые VPN-сервисы) | Реклама VPN-сервисов в России запрещена: штрафы и ответственность вплоть до статьи. Не заводить и не предлагать независимо от спроса. | 29.08.2026 | внесён 02.09.2026 |
| Ansys | Цен на сайте вендора нет (quote-only); узкоспециализированное дорогое инженерное ПО, которое не покупают через магазин формата BIZSoft. | 29.08.2026 | внесён 02.09.2026 |
| AWS | Не добавляем; ищем другие точки расширения линейки. | 01.09.2026 | внесён 01.09.2026, продублирован во второй реестр 02.09.2026 |

### Почему NordVPN и Ansys предлагались повторно

Разбор 02.09.2026 по замечанию руководителя («я ранее уже об этом писал, не
пойму, почему идёт сбой и повторная рекомендация»). Причин было две, обе
устранены:

1. **Решение 29.08 не дошло до системы.** Оно было записано только в этот
   markdown-файл. Ни один контур markdown не читает, поэтому исследование
   продолжало мерить спрос по обоим брендам и выносить их в блок «проверить
   вручную» — механизм отказа существовал (`audience.skip_reason`), но его
   реестр `wordstat/decisions.json` про эти бренды не знал.
2. **Реестра было два, и они не знали друг о друге.** Отказ по AWS от
   01.09 лёг в `intelligence/vendor-decisions.json` — его читает письмо, но не
   читает исследование; отказы по Spotify и ChatGPT Plus лежали в
   `wordstat/decisions.json` — наоборот. Решение, записанное «не в тот» файл,
   работало вполсилы.

Что изменено в коде: `audience.rejected_brands()` и
`report_v4.rejected_vendor_brands()` теперь читают оба реестра; на каждое
направление есть тест (`test_research_quality.py`,
`test_management_actions.py`). Дополнительно исправлена запись отката
эксперимента в реестр отклонённого (`committee.py` читал ключ `decision`,
а журнал пишет `owner_decision`).
...
| Atlassian | Jira Enterprise, Confluence Enterprise, Atlassian Guard | Enterprise-тарифы и корпоративный модуль управления доступом: цена и условия по индивидуальному расчёту через отдел продаж (quote-only). | https://www.atlassian.com/software/jira/pricing |
| Postman | Postman Enterprise | Enterprise-тариф вендора — не публикуем по правилу допуска §2, оформление через отдел продаж с годовым контрактом. | https://www.postman.com/pricing/ |
| TeamViewer | TeamViewer Tensor (все конфигурации) | Корпоративная линейка с условным доступом, единым входом и индивидуальным контуром — только по отдельному договору с вендором (quote-only). | https://www.teamviewer.com/en-us/pricing/overview/ |
| BrowserStack | Automate, App Automate, Automate TurboScale | Оплата за параллельные потоки с объёмной скидкой «lower price per parallel with higher volume» — фиксированного публичного SKU нет, расчёт индивидуальный. | https://www.browserstack.com/pricing |
| BrowserStack | Live Team Ultimate | Верхний командный тариф с объёмным ценообразованием («pay up to 80% less per user with volume-based pricing») — цена зависит от объёма, фиксированной карточки не получается. | https://www.browserstack.com/pricing?product=live |
| ManageEngine | ServiceDesk Plus Enterprise Edition | Enterprise-редакция — не публикуем по правилу допуска §2; считаем по запросу вместе с крупными пакетами специалистов. | https://store.manageengine.com/service-desk/ |
| n8n | n8n Business, n8n Enterprise | Business — модель с лицензионным ключом для собственной инфраструктуры (другая схема поставки), Enterprise — quote-only. | https://n8n.io/pricing/ |
| OpenRouter | Весь сервис как подписка | Подписок не существует: модель — предоплаченные кредиты с комиссией 5,5% за пополнение, неизрасходованные кредиты сгорают через год. Годового тарифа нет, продавать как подписку нечего. Возможная альтернатива — разовая услуга «пополнение баланса по договору», требует отдельного решения руководителя. | https://openrouter.ai/docs/faq |

## Lansweeper: отказ снят 02.09.2026

Запись об отказе действовала несколько часов: автоматическая проверка не смогла снять страницу тарифов (ops-probe — HTTP 403 с `cf-mitigated: challenge`, ops-price-scan с рендером Chrome — пустой результат, сайт закрыт Cloudflare). Руководитель проверил тарифы вручную и передал цены: Starter — фиксированно 2 000 устройств, от 3 500 долларов в год; Pro — от 2 000 до 9 000 устройств, от 7 000 долларов в год. Цены приняты как себестоимость закупки и вынесены на витрину по штатной формуле `computePegRub()` со стандартным коэффициентом. Вендор заведён: `src/data/vendors.ts`, пакет `scripts/catalog/lansweeper.json`, аудит цен — `reports/vendors/pricing-audit.md`.
