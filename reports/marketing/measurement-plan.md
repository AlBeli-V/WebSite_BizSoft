...
| `view_item` | открытие карточки `/product/[slug]` | — | G ✓ | `sku`, `vendor`, `category`, `price` | micro |
...
| `contact_click` | клик по телефону / e-mail / Telegram / WhatsApp | цель JS ✓ | G ✓ | `channel` (`phone`\|`email`\|`telegram`\|`whatsapp`), `page_url` | intent |

`contact_click` — **новое обязательное событие**. Сейчас не отправляется вовсе,
хотя `ContactDirect` присутствует под каждой формой: значительная доля B2B-
обращений уходит мимо форм, и этот канал сегодня невидим.
...
| Event | Trigger | М | G | Параметры | Уровень |
|---|---|---|---|---|---|
| `form_error` | ответ API 4xx/5xx или ошибка сети | — | G ✓ | `form_source`, `http_status`, `error_code` | диагностика |
