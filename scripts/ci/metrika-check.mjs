 ...
 *
 * Отсюда предмет проверки: не «отдаётся ли файл», а «запустился ли
 * счётчик». В локальном режиме признак — вызов ym(id, 'init', …) на
 * подменённом теге; в живом — обращения самого счётчика к своему домену
 * (хит, вебвизор, синхронизация кук): их делает только исполнившийся файл.
...
try {
  ...
  for (const path of PAGES) {
    ...
    page.on('request', (r) => {
      ...
      // Любое обращение счётчика помимо самого файла тега: хит, вебвизор,
      // синхронизация кук. Их делает только исполнившийся тег, поэтому это
      // и есть признак «счётчик запустился» — при инциденте не было ни
      // одного из них, потому что до исполнения дело не доходило.
      if (u.includes(TAG_HOST) && !u.includes(TAG_PATH)) counterCalls.push(
        u.replace(/^https:\/\/mc\.yandex\.ru/, '').replace(/\?.*/, ''));
    });
    ...
    if (LIVE) {
      ...
      if (!delivered) {
        ...
        // Предмет проверки — запустился ли счётчик. Исполнившийся тег сам
        // идёт на свой домен: хит, вебвизор, синхронизация кук. При
        // инциденте не было ни одного такого обращения, потому что файл не
        // исполнялся вовсе.
        const uniq = [...new Set(counterCalls)];
        report(`${path}: счётчик запустился (сам пошёл на свой домен)`, uniq.length > 0,
          uniq.join(', ') || 'ни одного обращения помимо файла тега');
        report(`${path}: ym определён на странице`, state.ymType === 'function',
          `typeof ym=${state.ymType}`);
        // Хит визита — отдельной строкой и без влияния на вердикт: из
        // headless-браузера в зарубежной сети Метрика его не всегда шлёт,
        // а поломку счётчика ловит признак выше. Настоящий контроль записи
        // визитов — пульс сайта в отчёте ops-direct stats.
        console.log(`     хит визита: ${hits.length ? `${hits.length} запрос(ов) к watch` : 'не зафиксирован'}`);
        if (!uniq.length) {
