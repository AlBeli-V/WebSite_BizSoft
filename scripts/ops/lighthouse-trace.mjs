 ...
 *              Шаблон — как у Lighthouse: путь со звёздочками по краям,
 *              например звёздочка + /m/tag.js + звёздочка. Записан он тут
 *              словами намеренно: пара «звёздочка со слешем» закрыла бы
 *              этот комментарий, и файл перестал бы разбираться (CI #400).
 *              Так меряется цена стороннего скрипта, не трогая живой сайт.
 *              Прогон с блокировкой — опыт, а не показатель сайта, поэтому
 *              шаблоны печатаются в заголовке каждого прогона и в итоге.
 *
