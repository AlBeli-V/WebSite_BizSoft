...
Решение владельца: привести Apple к тому же режиму целиком — показы
пн–пт 9:00–19:00. Вечерние часы 19:00–21:00 задавались отдельным решением
05.09 под предположение о частном спросе на карты; почасового разреза,
который подтвердил бы это предположение, в отчёте нет, а доля
информационного класса в расходе обеих кампаний держится на 69–72 %.
Окно приводится к общему для кабинета, различие окон больше не
поддерживается недоказанной догадкой.
...
WORKDAYS = (1, 2, 3, 4, 5)
# Целевое окно — то же, что у bs-test. Не переносится из текущего
# расписания: приводим к общему режиму кабинета, а не к тому, что стояло.
TARGET_HOURS = (9, 19)
...
def main() -> None:
    ...
    for name in CAMPAIGNS:
        ...
        hour_from, hour_to = TARGET_HOURS
        want = {d: (hour_from, hour_to) if d in WORKDAYS else None for d in range(1, 8)}
        have = {d: hours_of(by_day.get(d, [])) for d in range(1, 8)}
        if have == want:
            print("  = расписание уже целевое — менять нечего")
            continue
