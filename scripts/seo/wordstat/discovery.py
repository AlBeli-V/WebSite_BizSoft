...
PATTERNS = [
    ...
    # Корпоративные формы, добавленные 21.09.2026 после разбора рекламы.
    # Прежний вывод «корпоративные запросы не набирают» был сделан по одной
    # форме — «{vendor} для юридических лиц», 35 пустых ответов из 37. А в
    # запросах Вебмастера такие фразы есть, и их много: 301 запрос с признаком
    # юрлица дал 2233 показа за 11 дней. Просто словоформа другая: люди
    # набирают «оплата artlist юридическим лицом», «оплата capture one
    # юридическим лицом», «купить heygen юридическим лицом», «adobe купить для
    # компании», «корпоративные лицензии atlassian».
    #
    # Это единственные фразы, по которым к нам приходят заявки: 74% кликов
    # органики против 8% в рекламе (reports/marketing/ads-vs-organic-2026-09-21.md).
    # Априорные оценки продуктивности — по доле таких форм среди запросов
    # Вебмастера; статистика уточнит их фактом после первого прогона.
    ("оплата {vendor} юридическим лицом", 0.60,
     "рабочая корпоративная форма: так ищут те, кто оставляет заявки"),
    ("купить {vendor} для компании", 0.40,
     "вторая по частоте корпоративная форма в запросах Вебмастера"),
    ("корпоративная подписка {vendor}", 0.30,
     "корпоративный интент без слова «оплата»"),
]
PRODUCT_NAMES = {
