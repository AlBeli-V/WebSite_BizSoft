...
LIMITS = {
    ...
    # 1700 → 1850 (02.09): секция «Заявки за сутки» — заявки воронки, канал
    # каждой и путь клиента к запросу (вопрос руководителя от 01.09.2026
    # «из каких каналов приходили заявки»). Подробно раскрываются три
    # заявки, остальные — строкой, поэтому прибавка ограничена.
    # 1850 → 1900 (05.09): секция «Техническое состояние» — компактный блок
    # скорости в 5–7 строк при исправном сайте, подробности печатаются
    # только при просадке и не более чем по трём страницам.
    # 1900 → 2150 (14.09): секция «Аудитория: устройства и каналы» —
    # четыре плитки и две таблицы визитов по каналам (постановка
    # руководителя 14.09.2026). Замер на полном составе блока — 220 слов,
    # запас взят на четвёртый класс устройств и строки «нет данных».
    # 2150 → 2400 (16.09, решение руководителя): новых секций не появилось,
    # прибавку дают строки, число которых зависит от данных дня — раскрытые
    # классы каналов, эксперименты на контрольной дате, решения к ответу.
    # Замеры на неизменном составе: 2000 (14.09), 2138 (15.09), 2214 (16.09),
    # то есть запас прошлой правки ушёл за два дня. 2400 покрывает тот же
    # темп примерно на неделю; если письмо упрётся снова, вопрос не в
    # потолке, а в том, какая детализация уходит в веб-отчёт.
    "visible_words": 2400,
