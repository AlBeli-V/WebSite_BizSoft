...
CONTOURS = [
    ...
    # Витрина воронки — новый контур (02.09.2026): молчаливый сбой SSH или
    # Directus иначе виден только косвенно, по внезапно опустевшему блоку
    # первичных обращений, и пустые сутки от несобранных данных не отличить.
    # required=False до первого прогона на проде.
    #
    # В названии нет стема «заяв» намеренно: ярлык контура попадает в таблицу
    # loop-health письма комитета, а гейт NO_CRM (committee.py, FORBIDDEN_WORDS)
    # запрещает этот стем во всём тексте письма. 07.09.2026 письмо решений из-за
    # прежнего названия не собралось вовсе — гейт сработал на служебном ярлыке,
    # хотя адресован содержательным утверждениям о клиентах.
    {"id": "leads", "label": "Выгрузка витрины воронки (leads)",
