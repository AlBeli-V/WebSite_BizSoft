...
def _crm_entry(snap: dict) -> dict:
    """Обращения воронки сайта — пятая сущность измерения.

    Подключена с 01.09.2026 (ops-leads-collect). Сравнивать её с целевыми
    событиями Метрики напрямую нельзя: событие фиксирует действие в браузере
    (в том числе клик по телефону и повторную отправку), заявка — запись в
    воронке. Это разные сущности, а не расхождение источников.
    """
    crm = snap.get("crm") or {}
    ...
    return _entry(
        "Воронка сайта (Directus)", "leads", "обращение",
        "заявки сайта", f"{window.get('from')}–{window.get('to')}",
        "мусор (status=spam) исключён",
        "все формы сайта и скачивания КП",
        [],
        "Целевое событие Метрики фиксирует действие в браузере, заявка — "
        "запись в воронке; числа не равны и сведению не подлежат. Сделки и "
        "выручка не измеряются.",
        block.get("week_count"), "ok")
