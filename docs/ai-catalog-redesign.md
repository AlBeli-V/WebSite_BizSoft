...
- [x] **Витрина флагманов на хабе** — чипы в порядке приоритета Этапа 2; существование карточки проверяется в рантайме (`getProductsBySlugs`), до импорта ссылка ведёт на подкатегорию — 404 исключены.
- [x] **Канонизация URL подкатегорий** — `/catalog/ai-text` (плоский slug Directus) отдаёт 301 на `/catalog/ai/text`; sitemap выводит AI-категории только по вложенным URL.
- [x] **Футер** — ссылки «AI-сервисы для бизнеса» и «Решения под задачу».
- [x] **4-я KB-статья** — «Как выбрать AI-ассистента для команды разработки» (+ включена в перелинковку GitHub/Cursor).
- [x] **Перелинковка карточки** (Этап 5) — блок «Сравнения / База знаний / Категория» в `product/[slug].astro`, данные — `src/data/ai-interlinks.ts` (по вендору). «Похожие товары» (≥4) — через `related_products` в Directus.
- [x] «Сравнение тарифов» для карточек — решено без изменения схемы БД: таблицы тарифов живут на лендингах вендоров (`VendorLanding.astro`, поле `comparison` в `scripts/content/*.json`) и на страницах `/compare/*`, карточка ссылается на них из блока перелинковки.
