...
- [ ] Роут-хаб `/catalog/ai` (витрина флагманов + плитки) и `/catalog/ai/[sub]`.
- [ ] Блоки карточки: «Сравнение тарифов», «Почему через Biz-Soft», «Похожие/Статьи/Сравнения/Категория».
- [ ] `src/data/comparisons.ts` + `src/pages/compare/[slug].astro` (+ FAQ/Breadcrumb JSON-LD).
- [ ] Расширить `src/data/solutions.ts` девятью сценариями (Этап 7).
- [ ] 4 KB-статьи в `src/content/blog/`.
- [ ] Обновить `sitemap.xml.ts` и внутреннее меню/футер ссылками на подкатегории.
