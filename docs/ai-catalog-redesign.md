...
- [ ] Роут-хаб `/catalog/ai` (витрина флагманов + плитки) и `/catalog/ai/[sub]` — **зависит от категорий Directus** (раздел A).
- [ ] Блоки карточки: «Сравнение тарифов», «Похожие/Статьи/Сравнения/Категория» — в шаблоне `product/[slug].astro`.
- [ ] Внутреннее меню/футер — ссылки на подкатегории (после создания категорий).
