...
> **08.09.2026 ежедневное копирование остановлено по решению руководителя.**
> Строка снята из crontab сервера (`ops-backup` в режиме `uninstall`); скрипт,
> настройки и уже снятые архивы остались на месте. Последний архив —
> `/opt/bizsoft/backups/bizsoft-20260908-0030.tar.gz` (15 МБ, 08.09 03:30 МСК),
> всего 13 архивов с 26.08.2026, 146 МБ. Новых копий не снимается, писем по
> итогу прогона не приходит — молчание почты больше не сигнал. Возобновление —
> `ops-backup` в режиме `install` с ветки `main`. Всё, что описано ниже,
> относится к восстановлению из архивов, снятых **до** 08.09.2026.


| Показатель | Значение | Чем обеспечен |
|---|---|---|
| RPO (сколько данных теряем) | **не определён** — копирование остановлено 08.09.2026 | до 08.09.2026 — ежедневная копия в 03:30 МСК (`ops-backup`); сейчас новых копий нет |
...
- **08.09.2026 выгрузка в облако не работает:** refresh-токен Google протух
  (`invalid_grant: maybe token expired`), Диск не отвечает даже на чтение
  списка файлов, поэтому дату последней успешной выгрузки из сессии
  установить нельзя — её видно только в самой папке `BizSoft-Backups`.
  Это тот самый отказ, о котором предупреждает §9: приложение Google в режиме
  Testing протухает токен через 7 дней. Восстановление доступа —
  `rclone config reconnect gdrive:` на ноутбуке и новое значение в секрет
  `GDRIVE_RCLONE_CONF`, приложение — перевести в In production.
...
- **Ежедневный прогон остановлен 08.09.2026** (решение руководителя, строка
  снята из crontab через `ops-backup uninstall`). Пункты ниже описывают
  регламент, действующий при включённом копировании, и вступают в силу снова
  после `ops-backup install`.
