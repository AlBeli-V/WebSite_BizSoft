...
- **Ежедневно 07:00 МСК — сторож `ops-backup-watch`.** Заведён 14.09.2026
  после того, как истёкший OAuth-токен Google Drive тихо остановил выгрузку и
  пропуск заметили только по дыре в списке копий между 08.09 и 13.09. Сторож
  читает три вещи: возраст последней копии на сервере, строки с отметкой `✗` в
  осмотре `backup.sh check` и возраст последней копии в облаке. Молчит, пока
  всё в порядке; при находке шлёт письмо и пишет в issue #22, прогон при этом
  красный. Полагаться на ночное письмо как на единственный сигнал больше не
  нужно, но и отменять его не следует: сторож проверяет результат, письмо —
  ход прогона.
