...
> Расписание `ops-backup` включается только после того, как вручную прошли
> прогон `check`, первый `backup` и учебное восстановление из облачной копии.
> Пока расписание закомментировано, копия снимается ручным запуском, и
> фактический RPO равен времени с последнего запуска, а не 24 часам.
...
- Выгрузку делает `rclone` с сервера; его конфигурация (с refresh-токеном
  ...
  секрет: кто им владеет, тот читает и пишет весь Диск.
...
- **Ежедневно 03:30 МСК** — `ops-backup` снимает копию, проверяет её
  ...
  Google Drive (`BizSoft-Backups`). Отчёт — комментарием в issue #22. Прогон
  помечен `✗` — разбираться в тот же день: копии, о которой узнали в момент
...
| `GDRIVE_RCLONE_CONF` | конфиг rclone (base64) для выгрузки в Google Drive | то же самое |
