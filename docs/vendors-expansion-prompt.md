...
сам, курс не хардкодить ни в тексте, ни в цене); валюта в пакете —
валюта закупки, одна из двух (`base_price_eur` — с 15.09.2026,
`scripts/import-vendors.mjs`), пересчёт евро в доллары запрещён; учитывать период, годовое
...
- **краткое описание тоже различает.** `short_description` — лид карточки,
  `Product.description` в разметке, описание в фидах и источник, из которого
  `ops-fix-meta` собирает `meta_description`; одинаковый текст у соседних
  позиций превращается в дубли описаний по всему кластеру. Общий текст
  линейки разводится штатно:
  `node scripts/seo/dedupe-descriptions.mjs <файл-со-слагами> --write --sync-catalog`
  — различитель берётся из названия (редакция, объём, модуль, тип лицензии),
  тексты ложатся в партию `data/seo/product-descriptions.json` и в сам пакет
  каталога, чтобы следующий импорт не вернул дубль;
- **проверка автоматическая.** `tests/catalog-uniqueness.test.ts` роняет
  сборку на новой группе одинаковых названий И одинаковых кратких описаний,
  а также на росте уже известных групп (`data/seo/known-duplicate-names.json`
  — зафиксированный долг, который разгребается партиями, а не пополняется).
  Тот же тест проверяет партии текстов в
  `data/seo/product-descriptions.json`: уникальность `meta_title`,
  `meta_description` и `short_description`, длины 60/160, отсутствие пустых
  значений. Логика разведения покрыта `tests/dedupe-descriptions.test.ts`;
- **на проде сторожит `ops-content-audit`** — раз в неделю (понедельник,
  ...
  Разбирая долг, запускают с `full=true` — тогда печатается весь перечень
  групп со слагами, а не первая десятка;
