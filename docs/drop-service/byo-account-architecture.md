...
## 8. Решение по первому клиенту

Первым делается **расширение браузера** (решение руководителя 09.09.2026).
Первая версия закрывает разовый разбор кандидата: подключение ключа,
подтверждение владения, исполнение заданий с адреса пользователя, расход и
выгрузка. Регулярный мониторинг без открытого браузера и работа с кабинетом
регистратора остаются за локальным агентом второй версии.

Устройство расширения, инвариант «задание описывает намерение, а не запрос»
и приёмка — `docs/drop-service/extension-spec.md`.
