...
> **Статус на 08.09.2026: копирование остановлено по решению руководителя.**
> Строка снята из crontab (`ops-backup` в режиме `uninstall`), новых копий не
> снимается и писем по итогу прогона не приходит. Уже снятые архивы остались
> на сервере (13 штук с 26.08.2026, последний — 08.09 03:30 МСК). Выгрузка в
> Google Drive к этому моменту не работала: протух refresh-токен
> (`invalid_grant`). Возобновление — `ops-backup` в режиме `install`.
> Раздел ниже описывает контур в рабочем состоянии.

Копию снимает **сам сервер**: `/opt/bizsoft/backup.sh`, запускаемый ночным
cron в 03:30 МСК. Содержимое — дамп PostgreSQL, медиатека Directus,
`astro.env` и compose-файлы сервера → архив в `/opt/bizsoft/backups` и его
шифрованная копия в Google Drive (`BizSoft-Backups`, через rclone, с
обязательной сверкой sha256). Каждый прогон проверяет дамп накатом во
временный контейнер Postgres. Правило хранения на сервере — 7 ежедневных,
4 еженедельных, 6 ежемесячных; в облаке ротации нет намеренно (в скрипте нет
ни одной команды удаления в облаке), папка чистится вручную.

Расписание живёт в cron сервера, а не в GitHub Actions: 27.08.2026 планировщик
GitHub на шесть часов перестал выдавать запуски по расписанию всему
репозиторию, и ночная копия не снялась. События `schedule` в Actions не
гарантированы.

Управление — workflow `ops-backup` (только с ветки main, режим по умолчанию
`check`, то есть осмотр без записи): `check` показывает состояние и строку
crontab, `backup` снимает копию вне очереди, `install` заливает свежий скрипт
с настройками и ставит расписание, `uninstall` снимает расписание. Любой режим
обновляет скрипт на сервере из репозитория. Отчёт ручного прогона —
комментарием в issue #22, отчёт ночного — письмом на `avbelyaev@biz-soft.pro`
(логи в `/opt/bizsoft/backups/logs/`). Правило простое: письма нет — значит
cron не отработал.

Полная процедура восстановления — [`DR-RUNBOOK.md`](./DR-RUNBOOK.md): потеря
сервера, повреждение базы, точечный возврат удалённых записей, RPO/RTO,
регламент проверок и способ достать копию из Google Drive браузером, без
rclone. Нужные секреты репозитория: `BACKUP_PASSPHRASE` (шифрование архива) и
`GDRIVE_RCLONE_CONF` (конфиг rclone в base64). Любого из них нет — архив
остаётся только на сервере, прогон помечается неуспешным, старые копии при
этом не удаляются.
