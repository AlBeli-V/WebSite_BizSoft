...
Копии снимает workflow `ops-backup` (запуск вручную — только с ветки main,
режим по умолчанию `check`, то есть осмотр без записи): дамп PostgreSQL,
медиатека Directus, `astro.env` и compose-файлы сервера → архив в
`/opt/bizsoft/backups` и его шифрованная копия в Google Drive
(`BizSoft-Backups`, через rclone, с обязательной сверкой sha256). Правило
хранения на сервере — 7 ежедневных, 4 еженедельных, 6 ежемесячных; в облаке
ротации нет намеренно (в workflow нет ни одной команды удаления в облаке),
папка чистится вручную. Каждый прогон
проверяет дамп накатом во временный контейнер Postgres; отчёт — комментарием
в issue #22. Расписание пока отключено намеренно, включается после первого
успешного `backup` и учебного восстановления.
