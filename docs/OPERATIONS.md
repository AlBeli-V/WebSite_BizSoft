...
Копии снимает workflow `ops-backup` (запуск вручную и только с ветки main): дамп
PostgreSQL, медиатека Directus, `astro.env` и compose-файлы → архив в
`/opt/bizsoft/backups` и его шифрованная копия в Google Drive, папка
`BizSoft-Backups`. На сервере хранятся 7 суточных, 4 недельных и 6 месячных копий
(поле `retention`, по умолчанию `7/4/6`); в Google Drive ротации нет. Каждый прогон
проверяет дамп накатом во временный контейнер Postgres; отчёт — комментарием в
issue #22.

Режим `mode=check` ничего не пишет: показывает контейнеры, размер базы, место на
диске, заданность секретов и доступность Google Drive. Ночное расписание в
workflow закомментировано — включать после успешных ручных прогонов `check` и
`backup`.
...
`GDRIVE_RCLONE_CONF` (содержимое `rclone.conf` с доступом к Google Drive). Не задан
любой из двух — копия остаётся только на сервере, и прогон помечается неуспешным;
уже снятые копии при этом не удаляются.
