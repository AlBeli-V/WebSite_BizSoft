...
| robots.txt и GET-параметры | `scripts/seo/robots-lib.mjs`, `robots-sync.mjs`, `public/robots.txt` | реестр `data/seo/url-params.json` (незначащие метки → `Clean-param`, значащие → чем закрыт дубль) | `pnpm robots:sync` (сборка), `ops-schema-check` (недельная сверка с продом) | `docs/rules/url-params.md` | `tests/robots.test.ts`, `pnpm check:robots` |
