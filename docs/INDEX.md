...
| SEO Growth Intelligence | `scripts/seo/*.py` (collect, snapshot, quality, report_v4, webreport, allocator, loop_health) | ветка `seo-data` → `reports/seo/*` | `seo-data-collect`, `seo-daily-report`, `seo-report-email`, `seo-period-report`, `seo-site-check`, `seo-analytics-check`, `seo-goals-sync`, `seo-publish-web` | `reports/seo/README.md`, `docs/seo/reporting-methodology.md` (721 стр. — по разделу), `docs/seo/goals.md` | `python3 -m unittest discover -s scripts/seo/tests` (94 проверки) |
...
| WebMCP | `src/webmcp/*`, `src/pages/api/agent/*` | тот же Directus, отдельной базы нет | входит в `ci.yml` | `docs/webmcp/architecture.md`, `security.md`, `tools.md`, `testing.md` | `pnpm test:webmcp-browser` |
