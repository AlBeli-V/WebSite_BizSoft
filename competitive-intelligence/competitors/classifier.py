...
# сайты вендоров, а также неклассифицированные домены («?»). Все они забирают
# клик, но считать их соперниками за сделку оснований нет: по одним это видно
# из типа площадки, по другим — просто ничего не известно.
#
# Правило для «?» задано явно (1.3.0): за клик — да, за сделку — нет.
# Несимметрично намеренно. Ошибка в сторону «за клик» безобидна: домен
# попадает в перечень тех, кто стоит выше нас, и мы это видим. Ошибка в
# сторону «за сделку» дороже: неизвестный домен стал бы главным конкурентом
# письма без единого подтверждённого признака. Неклассифицированные домены
# видны отдельным счётчиком в снимке («не_классифицировано») и подлежат
# ручной проверке — молча в рейтинг они не попадают.
...
def in_main_ranking(category: str) -> bool:
    ...
    теста на ложные срабатывания из раздела 32 задания. Неклассифицированный
    домен («?») тоже не входит: о нём известно слишком мало, чтобы называть
    его соперником за сделку.
    """
