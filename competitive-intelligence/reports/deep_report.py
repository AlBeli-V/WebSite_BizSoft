...
def _google_hypothesis(profile: dict) -> str:
    """Разрыв с Google, разложенный до проверяемой гипотезы и поручения.

    До 1.9.3 отчёт называл разрыв главным вопросом, перечислял три гипотезы —
    индексация, авторитет домена, конкурентоспособность страницы — и на этом
    заканчивал: гипотезы не ранжировались, проверка не предлагалась, поручений
    не выдавалось. Самая дорогая находка дня не приводила ни к одному
    действию.

    Разводит гипотезы величина, которая уже собрана: встречается ли наша
    страница в Google-срезе хоть где-нибудь.
    """
    всего = profile.get("страниц_в_разрыве") or 0
    if not всего:
        return ""
    нет = profile.get("страниц_нет_в_срезе") or 0
    есть = profile.get("страниц_есть_в_срезе") or 0
    доля = нет / всего
    первые = profile.get("первые") or []
    # Порог намеренно высокий: гипотеза о видимости страницы отвергается
    # только тогда, когда её не видно почти нигде. При смешанной картине
    # вывод не делается — называются обе части.
    if доля >= 0.9:
        вывод = (
            f'<b>Проверять первой надо видимость страниц, а не их качество.</b> '
            f'Из {plural(всего, "страницы", "страниц", "страниц")} нашего сайта, '
            f'попавших в разрыв, в Google-срезе не встречается ни разу '
            f'{esc(нет)} — по любому запросу, на любой позиции его глубины. '
            f'Страница, проигрывающая конкурентам по релевантности, стоит на '
            f'11–20 месте и в срез попадает. Здесь её нет вовсе.')
    elif доля <= 0.3:
        вывод = (
            f'<b>Страницы Google видит — вопрос в том, как он их ранжирует.</b> '
            f'Из {esc(всего)} страниц разрыва {esc(есть)} встречаются в срезе '
            f'по другим запросам, то есть в выдачу попадают, но не в топ-10 по '
            f'этим. Это профиль конкурентоспособности, а не видимости.')
    else:
        вывод = (
            f'<b>Картина смешанная, одной гипотезы не хватает.</b> Из '
            f'{esc(всего)} страниц разрыва {esc(есть)} в срезе встречаются, '
            f'{esc(нет)} — ни разу. Разбирать эти две группы надо порознь: у '
            f'них разные причины и разные проверки.')
    строки = "".join(
        ...
        f'<td>{"есть" if p["есть_в_google_срезе"] else "нет ни разу"}</td></tr>'
        for p in первые)
    таблица = ('<div class="scroll"><table><tr><th>Наша страница</th>'
               '<th class="num">Запросов в разрыве</th>'
               '<th class="num">Лучшая позиция в Яндексе</th>'
               '<th>В Google-срезе</th></tr>' + строки + '</table></div>')
    сколько = (f'{len(первые)} из {всего}' if len(первые) < всего
               else str(всего))
    поручение = (
        f'<p class="lead"><b>Что с этим делать.</b> Проверить в Google Search '
        f'Console индексацию страниц из списка ниже — {esc(сколько)}, начиная '
        f'с верхней: они отсортированы по числу запросов, которые каждая '
        f'держит. Это проверка, а не правка: пока не известно, в индексе '
        f'страница или нет, любая доработка текста под Google — работа вслепую. '
        f'Приёмка: по каждой странице записан ответ Search Console — в индексе, '
        f'исключена (с причиной) или не обнаружена.</p>')
    return (f'<div class="card"><h4 id="google-why">Чьё это отсутствие: '
            f'страниц или домена</h4><p class="lead">{вывод}</p>'
            + поручение
            + cut("Страницы разрыва по весу", таблица,
...
def _google_hypothesis_why(profile: dict) -> str:
    """Обоснование под кат: как одна величина разводит три гипотезы."""
    всего = profile.get("страниц_в_разрыве") or 0
    нет = profile.get("страниц_нет_в_срезе") or 0
    наших = profile.get("наших_url_в_срезе_всего") or 0
    return (
        f'<p class="q"><b>Что именно измерено.</b> Google-срез прочитан '
        f'целиком: по всем запросам и на всю глубину. Из него собраны все '
        f'адреса нашего сайта, встреченные где угодно, — таких '
        f'{plural(наших, "адрес", "адреса", "адресов")}. Затем взяты наши '
        f'страницы, которые в Яндексе держат топ-10 по запросам разрыва: их '
        f'{esc(всего)}. Пересечение этих двух множеств и отвечает на вопрос, '
        f'видит ли Google страницу вообще.</p>'
        f'<p class="q"><b>Почему это разводит гипотезы.</b> Три объяснения '
        f'разрыва дают разные наблюдаемые следствия. Слабая '
        f'конкурентоспособность страницы означает, что страница в выдаче есть, '
        f'но ниже: она попадает в собранную глубину по части запросов и видна '
        f'в срезе. Недостаток авторитета домена действует так же — ровнее и '
        f'ниже, но присутствие сохраняется. И только отсутствие страницы в '
        f'видимой выдаче даёт ноль вхождений по всем запросам сразу. '
        f'Наблюдается третье: {esc(нет)} из {esc(всего)}.</p>'
        f'<p class="q"><b>Чего этот вывод не доказывает.</b> Отсутствие в '
        f'срезе — не то же самое, что отсутствие в индексе: страница может быть '
        f'проиндексирована и стоять ниже собранной глубины. Поэтому вывод '
        f'сформулирован как «проверять первой» — он назначает порядок проверок, '
        f'а не выносит диагноз. Порядок важен: проверка индексации бесплатна и '
        f'занимает минуты, а доработка текста под Google — недели работы '
        f'редактора, и начинать с неё значит платить за диагноз самую высокую '
        f'цену из возможных.</p>')
