...
def _google_hypothesis(profile: dict) -> str:
    ...
    группы = [
        ...
        ("знает, но не индексирует", "Google знает адрес и в индекс не берёт",
         "Адрес обнаружен, страница не проиндексирована. Прежде чем считать "
         "это приговором содержимому, посмотрите строку об обходе ниже: если "
         "страницу не скачивали, это очередь, а не оценка, и переписывать "
         "нечего."),
    ...
    обход = profile.get("обход_по_сайту") or {}
    не_скачано = profile.get("не_скачано") or 0
    строка_обхода = ""
    if обход.get("страниц"):
        доля = обход["не скачано"] / обход["страниц"]
        строка_обхода = (
            f'<p class="lead"><b>Сколько из этого — очередь обхода.</b> '
            f'Google ни разу не скачивал {esc(не_скачано)} из '
            f'{esc(profile.get("страниц_в_разрыве", 0))} страниц разрыва. '
            f'По сайту целиком не скачано {esc(обход["не скачано"])} из '
            f'{esc(обход["страниц"])} ({доля:.0%}), последний обход '
            f'{esc(обход.get("последний_обход", "—"))}. Страницу, которую '
            f'не скачивали, переписывать бесполезно: её содержимое Google '
            f'не видел.</p>')
...
основание ряда динамики: {esc(trend_basis or "не считался")}{esc(f" ({trend_note})" if trend_note else "")}.<br>
