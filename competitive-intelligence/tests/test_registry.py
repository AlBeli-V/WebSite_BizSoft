...
class TestGoogleSummary(unittest.TestCase):
    """Блок Google снимка дня: свой движок, свой регион, доля внутри поля."""

    def test_summary_по_google_срезу(self):
        ...
        rows = [
            serp_source.SerpRow(date="2026-09-03", query="купить figma",
                                ...
                                top=[{"domain": "ggsel.net", "url": "https://ggsel.net/"},
        ...
        g = run_discovery.google_summary(rows, config, "2026-09-03")
        self.assertEqual(g["запросов_с_данными"], 2)
        self.assertEqual(g["запросов_всего"], 3)
        self.assertEqual(g["глубина"], 10)
        self.assertEqual(g["гео"], "Россия (loc 2643)")
        self.assertEqual(g["топ10"], 1)
        self.assertEqual(g["лучшая_позиция"], 2)
        self.assertEqual(g["наши_запросы"], [{"запрос": "купить figma", "позиция": 2}])
        self.assertEqual(g["лидеры"][0]["домен"], "ggsel.net")
        self.assertGreater(g["лидеры"][0]["доля"], g["наша_доля_видимости"])
        self.assertGreater(g["наша_доля_видимости"], 0)

    def test_без_среза_none(self):
        from discovery import run_discovery
        from scoring import visibility
        self.assertIsNone(run_discovery.google_summary([], visibility.load_config()))
