...
# Сколько букв в конце формы разрешено считать окончанием: общее начало двух
# форм должно покрыть более короткую из них без последних двух букв. Меньше
# трёх букв общего начала не хватает никогда — там сливаются разные корни.
ENDING = 2
MIN_COMMON = 3
...
def stem(word: str) -> str:
    ...
    Оставлена для совместимости и для отладки. В проверке вхождений не
    участвует: см. `same_word` и разбор ниже.
    """
    word = word or ""
    return word if len(word) <= 4 else word[:max(4, len(word) - 2)]


def same_word(query_word: str, page_word: str) -> bool:
    """Одно ли это слово в разных формах.

    Почему не отсечение фиксированного числа букв (разбор отчёта 04.09.2026).
    `stem` режет ровно две последние буквы, и длина основы зависит от длины
    формы: «оплата» → «опла», «оплатой» → «оплат», «оплатить» → «оплати». Три
    разные основы у одного корня, причём у самого частого слова сайта. Из-за
    этого отчёт требовал «дописать слово оплатить» на странице, где формы
    «оплата» и «оплаты» стояли одиннадцать раз, — то есть поручал сделать
    сделанное. Пять из восемнадцати утверждений «не хватает слов» в отчёте за
    04.09 были такими.

    Сравниваем по общему началу двух форм, а не по обрезкам каждой в
    отдельности: «оплата» и «оплатить» совпадают на пяти буквах, «россии» и
    «россиян» — на пяти, «лицом» и «лицам» — на трёх. Порог берётся по более
    короткой форме: её начало без последних двух букв должно совпасть целиком.

    Порог сознательно мягкий. Ошибиться можно в две стороны, и они неравны:
    лишнее поручение — это работа руководителя впустую и подорванное доверие
    к списку, пропущенное — всего лишь несделанное улучшение, которое вернётся
    в отчёт назавтра. Поэтому спорные пары («лицо» и «лицензия») считаются
    одним словом, а не разными.
    """
    a = query_word or ""
    b = page_word or ""
    if a == b:
        return True
    need = max(MIN_COMMON, min(len(a), len(b)) - ENDING)
    common = 0
...
def coverage(page: PageContent, query: str) -> tuple[str, list[str]]:
    ...
    # Сравнение пословное, а не по подстроке: подстрока «карт» находится
    # внутри «карточки» и «открыть», и запрос про карту считался раскрытым
    # страницей, которая о картах не говорит вовсе.
    body = set(page.haystack.split())
    prominent = set(page.prominent.split())
    missing = [w for w in words if not any(same_word(w, p) for p in body)]
    if missing:
        return COVER_NONE, missing
    if all(any(same_word(w, p) for p in prominent) for w in words):
