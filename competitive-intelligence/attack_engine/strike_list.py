...
способная продать тому же покупателю, — на кону сделка. Если выше только
официальный сайт вендора, статья, маркетплейс или домен, который мы ещё не
классифицировали, — переход мы теряем точно, а вот сделку такой участник
забирает реже и другим способом: производитель может продать напрямую,
...
B2B-продажи у такого участника не подтверждено. Раньше такие запросы
выпадали из поля зрения целиком; теперь они остаются кандидатами с пометкой
вида конкуренции, а приоритет при равной выгоде отдаётся сделке.
"""
...
def build(rows, *, region: str = "213", engine: str = "yandex",
    ...
    for row in rows:
        ...
        # B2B-продажи (на кону сделка), и отдельно тот, у кого не
        # подтверждены (на кону как минимум переход). В 1.0.0 запрос
        # отбрасывался, если выше не было ни одного «делового» конкурента —
        # но если над нами официальный сайт вендора и статья, переход мы
        # всё равно теряем.
        business_rival = None
        serp_rival = None
        ...
        for index, item in enumerate(row.top[:RIVAL_POSITION_MAX], start=1):
            ...
            # Неклассифицированный домен («?») может стать соперником за
            # клик, но не за сделку: in_main_ranking его не пропускает.
