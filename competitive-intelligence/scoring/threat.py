...
def score(card: dict, *, history: list[float] | None = None,
    ...
    # Superiority: если счётчик «выше нас» не передан, используем присутствие
    # в ТОП-3 как приближение — но помечаем это в объяснении, чтобы
    # приближение не выдавалось за измерение.
