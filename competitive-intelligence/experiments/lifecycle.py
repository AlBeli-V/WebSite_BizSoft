...
def positions_on(snapshot: dict, queries) -> dict[str, int]:
    ...
    Отсутствие в собранной выдаче — не «нет данных», а позиция хуже её
    глубины: запрос измерялся, нас там не нашли. Поэтому подставляется
    «глубина + 1», иначе выпадение из выдачи улучшало бы среднее.

    Глубина берётся из самого снимка, а не из константы: срез Яндекса имеет
    глубину 10, и штраф за отсутствие равен 11, а не 21 — см. journal.
    out_of_top и разбор 10.09.2026.
    """
    ...
    for query in queries:
        ...
        if "позиция" not in bucket:
            continue
        result[query] = bucket["позиция"] or jr.out_of_top(snapshot)
    return result
