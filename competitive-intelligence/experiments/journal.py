...
# Позиция, которой заменяется отсутствие в собранной выдаче. Без такой замены
# страница, вылетевшая из выдачи, просто исчезала бы из расчёта и эксперимент
# выглядел бы успешнее, чем есть: считались бы только те запросы, где нам
# повезло.
#
# Значение берётся из глубины среза (глубина + 1), а константа ниже — только
# запасная для снимков, где глубина не сохранена. До 1.9.7 она была
# единственной и равнялась 21 при реальной глубине среза Яндекса 10:
# страница на 11-м месте получала штраф как за 21-е. Эффект правки считается
# по изменению медианной позиции, поэтому пересечение границы топ-10
# перевешивало всё остальное — уход с 9-го на 11-е выглядел падением на 12
# позиций вместо двух, а возврат — таким же взлётом. Ни один эксперимент ещё
# не доведён до оценки, поэтому исправление ничего не ломает задним числом.
OUT_OF_TOP = 21


def out_of_top(snapshot: dict | None = None) -> int:
    """Условная позиция «нас нет» для снимка: глубина среза плюс один."""
    depth = ((snapshot or {}).get("покрытие") or {}).get("глубина_яндекс")
    return int(depth) + 1 if depth else OUT_OF_TOP
