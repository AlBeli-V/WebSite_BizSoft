...
def pick(snapshot: dict, previous: dict | None = None) -> Signal:
    ...
    if previous and not comparable_cores(snapshot, previous):
        ...
        signal.text += (f" Сравнение с {previous.get('дата')} не приводится: "
                        f"ядро выросло с {core_was} до {core} запросов, "
                        f"доли по разным ядрам несопоставимы."
                        if (core or 0) > (core_was or 0) else
                        f" Сравнение с {previous.get('дата')} не приводится: "
