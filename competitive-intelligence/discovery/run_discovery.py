...
def build_snapshot(date: str, cards: list, rows: list, config: dict,
    ...
    return {
        ...
        "покрытие": {
            ...
            # Глубина собранной выдачи. Хранится в снимке, потому что
            # от неё зависит, что значит «нас нет»: при глубине 10
            # отсутствие — это «хуже десятого», а не «хуже двадцатого»
            # (разбор 10.09.2026).
            "глубина_яндекс": max((len(r.top) for r in usable), default=0),
            "взвешенное_покрытие": weighted_coverage,
