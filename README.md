...
(строка снята из crontab сервера через `ops-backup uninstall`). Уже снятые
архивы остались в `/opt/bizsoft/backups`, последний — от 08.09.2026; новых
копий не снимается. Возобновление — `ops-backup` в режиме `install` с ветки
`main`. Ниже — устройство контура в рабочем состоянии.

Копию базы Directus, медиатеки и секретов прода снимает cron самого сервера
(`/opt/bizsoft/backup.sh`, ежедневно в 03:30 МСК; управление — workflow
`ops-backup`, только с ветки `main`): архив в `/opt/bizsoft/backups` плюс шифрованная копия в
Google Drive (папка `BizSoft-Backups`, выгрузка через rclone), каждая копия
проверяется пробным восстановлением во временный Postgres. Хранится 7 ежедневных,
4 еженедельных и 6 ежемесячных копий. Процедура подъёма сайта после аварии —
`docs/DR-RUNBOOK.md` (его открытая копия лежит рядом с архивами в облаке как
`README-RESTORE.md`), аудит готовности — `reports/ops/dr-readiness-2026-08-24.md`.
