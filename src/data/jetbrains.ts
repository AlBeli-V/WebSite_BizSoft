...
export const JETBRAINS_LICENSE_MANAGEMENT: { title: string; text: string }[] = [
  ...
  { title: 'Сопровождение IT-отдела', text: 'Помощь при настройке, продлении и контроле лицензий силами BIZSoft.' },
];
...
export const JETBRAINS_OBJECTIONS: { q: string; a: string }[] = [
  { q: 'Дорого / непонятна цена', a: 'Цена зависит от продукта, числа пользователей и курса евро. Пришлём прозрачный расчёт в КП.' },
...
export const JETBRAINS_FAQ: { q: string; a: string }[] = [
  { q: 'Можно ли купить JetBrains на юридическое лицо?', a: 'Да. BIZSoft оформляет поставку по договору, выставляет счёт и предоставляет закрывающие документы (акт или УПД), работает через ЭДО.' },
  { q: 'Какие документы вы предоставляете?', a: 'Договор, счёт и УПД с выделенным НДС 5% (или акт со счётом-фактурой). Документооборот ведём через ЭДО.' },
  { q: 'Как формируется цена?', a: 'Цена — годовая подписка на пользователя; зависит от продукта, числа пользователей, типа лицензии и курса евро. ' },
  ...
  { q: 'Сколько стоит IntelliJ IDEA Ultimate для организации?', a: 'Цена указана на карточке как ориентир за пользователя в год.' },
