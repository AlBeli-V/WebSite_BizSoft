...
export const FIGMA_SUMMARY =
  'BIZSoft подбирает и оформляет подписки Figma для юридических лиц по новой модели посадочных мест: Full (дизайн), Dev (разработка) и Collab (просмотр и комментирование) на планах Professional, Organization и Enterprise. Доступны договор, счёт и закрывающие документы; оплата в рублях по курсу ЦБ.';
...
export const FIGMA_COMPARISON = {
  ...
  rows: [
    ...
    // Через BIZSoft все планы продаются готовыми годовыми тарифами.
    { label: 'Оплата через BIZSoft', values: ['1 год', '1 год', '1 год'] },
  ],
};
...
export const FIGMA_FAQ: { q: string; a: string }[] = [
  ...
  { q: 'Сколько стоит Figma?', a: 'Цена зависит от плана и типа мест. Стоимость в рублях считается от прайса Figma по курсу ЦБ РФ; актуальные цены — в карточках выше.' },
