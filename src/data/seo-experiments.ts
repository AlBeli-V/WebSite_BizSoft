...
export const SEO_EXPERIMENTS: Record<string, SeoExperiment> = {
  ...
  //
  // Остановлен досрочно решением руководителя 11.09.2026: страница уходит в
  // переработку, а два изменения сразу сделали бы вывод о сниппете
  // неотделимым от вывода о композиции. Вывод по CTR не снимается — это
  // принятая цена решения. Сниппет оставлен как есть (KEEP, так же закрывали
  // snippets-6-demand 03.09.2026): title написан под фактическую
  // формулировку кластера, и возвращать общий шаблонный заголовок незачем.
  // Снято только `faq`: типовые четыре вопроса подменяли собственный FAQ
  // страницы, а он отвечает ровно на то, о чём спрашивают перед сделкой —
  // чем Premium seat отличается от Standard, почему минимум два места,
  // уходят ли данные компании в обучение модели.
  anthropic: {
