/**
 * Единый источник реквизитов и настроек бренда BIZSoft.
 * Используется в подвале, контактах, документах, политике ПДн,
 * Organization/LocalBusiness-разметке и в PDF коммерческого предложения.
 * Меняем реквизиты — только здесь.
 */
import { VENDORS } from '../data/vendors';

export interface BankDetails {
  bankName: string;
  account: string; // расчётный счёт
  corrAccount: string; // корреспондентский счёт
  bik: string;
}

export interface SellerDetails {
  brand: string;
  legalName: string;
  shortName: string;
  address: string;
  phone: string;
  phoneHref: string;
  email: string;
  inn: string;
  ogrnip: string;
  okpo: string;
  okato: string;
  oktmo: string;
  registrationDate: string;
  bank: BankDetails;
}

/**
 * Ответственный за заявки по умолчанию.
 *
 * Распоряжение руководителя 21.08.2026: ответственный всегда Беляев Алексей.
 * Живёт в общем конфиге, а не в модуле CRM: заявку заводит и сайт (форма,
 * скачивание КП), а витрине импортировать из src/crm запрещено — границу
 * между CRM и сайтом стережёт tests/crm-isolation.test.ts.
 */
export const defaultLeadOwner = 'Беляев Алексей';

export const seller: SellerDetails = {
  brand: 'BIZSoft',
  legalName: 'Индивидуальный предприниматель Беляев Алексей Васильевич',
  shortName: 'ИП Беляев А.В.',
  address: '115569, Москва, Каширское шоссе 80К1',
  phone: '+7 (964) 716-11-11',
  phoneHref: '+79647161111',
  email: 'hello@biz-soft.pro',
  inn: '507202054051',
  ogrnip: '322774600665109',
  okpo: '2019116499',
  okato: '45296577000',
  oktmo: '45921000',
  registrationDate: '07.11.2022',
  bank: {
    bankName: 'АО «ОТП Банк», г. Москва',
    account: '40802810100510000665',
    corrAccount: '30101810000000000311',
    bik: '044525311',
  },
};

/**
 * Публичный эксперт BIZSoft: автор материалов и спикер для изданий.
 *
 * Решение руководителя 08.09.2026 (задача ENT-001). Требование к роли одно —
 * настоящий человек с настоящим участием в сделках; выдуманный автор
 * запрещён и обесценивает разметку авторства целиком. Отсюда берутся
 * `author` статей, страница автора и `founder` организации.
 */
export const expert = {
  slug: 'alexey-belyaev',
  name: 'Алексей Беляев',
  fullName: 'Беляев Алексей Васильевич',
  jobTitle: 'Основатель BIZSoft',
  /** Кратко — для подписи под статьёй. */
  tagline: 'Основатель BIZSoft, ведёт поставки зарубежного ПО российским юрлицам с 2022 года.',
  /** Развёрнуто — для страницы автора. */
  bio: 'Основатель BIZSoft. Занимается закупкой и оплатой зарубежного программного обеспечения и SaaS-сервисов для российских юридических лиц: подбор тарифа, договор, счёт в рублях, оплата вендору и закрывающие документы через ЭДО. Через эту практику проходят подписки на AI-сервисы, инструменты разработки, дизайн-ПО и корпоративные лицензии — материалы сайта пишутся из неё, а не из пересказов.',
  email: 'hello@biz-soft.pro',
} as const;

/**
 * Внешние профили организации для `sameAs` в разметке Organization.
 *
 * Задача ENT-001. Разбор 08.09.2026: у организации не было ни одного
 * внешнего подтверждения — Google по запросу собственного бренда «bizsoft»
 * ставил сайт на позицию 15,7, потому что различать омонимов ему нечем
 * (компании с таким именем есть и в других странах).
 *
 * Правило заполнения: сюда попадает только подтверждённый профиль, который
 * реально открывается и содержит те же название, юр-название, ИНН, адрес и
 * телефон, что и сайт. Ссылка на несуществующий или чужой профиль вредит
 * сильнее, чем её отсутствие, поэтому список стартует пустым и растёт по
 * мере регистраций (уровень 2 плана `reports/seo/google-entity-gap.md`).
 * Расхождение хотя бы в одном символе делает профиль не подтверждением, а
 * вторым кандидатом в омонимы.
 */
export const sameAs: string[] = [];

/**
 * Предметные области организации (`knowsAbout`). Это не ключевые слова, а
 * заявление о специализации: чем именно занимается лицо, стоящее за сайтом.
 */
export const knowsAbout: string[] = [
  'Закупка зарубежного программного обеспечения для юридических лиц',
  'Оплата зарубежных SaaS-сервисов из России',
  'Корпоративные подписки и лицензирование ПО',
  'AI-сервисы для бизнеса',
  'Электронный документооборот и закрывающие документы на ПО',
];

/**
 * Публичный почтовый адрес в структурированном виде — для Schema.org
 * (PostalAddress). Ровно те же данные, что видимая строка seller.address
 * на /contacts: номер офиса сайт намеренно не публикует, поэтому его нет
 * и в разметке.
 */
export const sellerAddress = {
  streetAddress: 'Каширское шоссе 80К1',
  addressLocality: 'Москва',
  postalCode: '115569',
  addressCountry: 'RU',
} as const;

/**
 * Режим работы (решение руководителя 29.08.2026: Пн–Пт 10:00–19:00 МСК).
 * Единый источник для /contacts и разметки LocalBusiness — чек-лист
 * товарных сервисов Яндекса требует явного графика на сайте.
 */
export const workingHours = {
  label: 'Пн–Пт 10:00–19:00 (МСК)',
  /** schema.org OpeningHoursSpecification */
  schema: {
    '@type': 'OpeningHoursSpecification',
    dayOfWeek: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
    opens: '10:00',
    closes: '19:00',
  },
} as const;

export const site = {
  name: 'BIZSoft',
  domain: 'biz-soft.pro',
  url: 'https://biz-soft.pro',
  tagline: 'Единая точка доступа к ПО и AI-сервисам',
  description:
    'BIZSoft (Business Integration Zone) — единая точка доступа к зарубежным ПО- и AI-сервисам для российских юрлиц: по договору, с оплатой по счёту и закрывающими документами через ЭДО.',
  // Срок действия коммерческого предложения по умолчанию (дней)
  // Срок действия КП — распоряжение руководителя 21.08.2026: неделя от даты
  // скачивания. Дальше цены вендоров и курс расходятся с расчётом.
  quoteValidDays: 7,
  // Дефолтная наценка при привязке к курсу (страховка, если не задана у товара)
  defaultMarkupPercent: 0,
} as const;

/**
 * Менеджер в письмах и на странице предложения.
 *
 * Не «отдел продаж», а конкретный человек с телефоном и лицом: B2B-переписку
 * ведут люди, и обезличенная подпись стоит клиенту лишнего шага «а с кем я
 * вообще разговариваю». Фотография необязательна — без файла блок показывает
 * инициалы, а не битую картинку.
 */
export const offerManager = {
  name: 'Алексей Беляев',
  role: 'Менеджер BIZSoft',
  email: 'hello@biz-soft.pro',
  /**
   * Почта в подписи письма. Отличается от `email`: общий ящик остаётся
   * адресом отправителя и адресом заготовок `mailto`, а под именем в подписи
   * стоит личный адрес менеджера — заказчик пишет человеку, а не в приёмную
   * (решение руководителя 21.09.2026).
   */
  signatureEmail: 'avbelyaev@biz-soft.pro',
  phone: '+7 (964) 716-11-11',
  phoneHref: '+79647161111',
  telegram: 'https://t.me/ALBE_RU',
  /**
   * Файл в public/. Пусто — в письме и на странице будут инициалы.
   *
   * 320×320 на белом фоне, 32 КБ: письмо обязано быть лёгким, а исходный
   * экспорт (1254×1254, 1,8 МБ) нёс запечённую «шахматку» удалителя фона —
   * в письме она выглядела бы браком вёрстки.
   */
  photo: 'brand/manager-belyaev.jpg',
  initials: 'АБ',
} as const;

/**
 * Электронный документооборот: Контур.Диадок.
 *
 * Публичной ссылки-приглашения у Диадока нет — приглашение отправляется
 * изнутри кабинета поиском по идентификатору участника ЭДО или по ИНН
 * (справка Контура). Поэтому клиенту мы отдаём сами значения, а не ссылку:
 * выдуманный deep-link в письме заказчика — это ошибка, которую он обнаружит
 * в момент, когда собирался с нами работать.
 *
 * `managerSearchUrl` — личный кабинет продавца, ссылка уходит только в
 * служебное письмо менеджеру: в адресе идентификатор его ящика, у клиента
 * она не откроется.
 */
export const edo = {
  provider: 'Контур.Диадок',
  participantId: '2BM-507202054051-20190514112005621867700000000',
  managerSearchUrl: 'https://diadoc.kontur.ru/810b23cc-c39a-4927-9b68-309ba169d487/Counteragents/Search',
  /** Порядок действий на стороне заказчика — из справки Контура. */
  howTo: 'Контрагенты → Пригласить нового → поиск по идентификатору участника ЭДО или по ИНН',
} as const;

/**
 * Типовые документы для ознакомления — один файл, а не комплект.
 *
 * Рамочный договор и дополнительное соглашение №1 собраны в один документ
 * (решение руководителя 15.09.2026): заказчику нужен один клик, а не архив,
 * который ещё надо распаковать. Файл лежит по прямой ссылке, со страниц
 * сайта на него не ссылаются и в индекс он не попадает.
 */
export const offerDocs = {
  contract: {
    path: '/documents/bizsoft-dogovor-i-ds.docx',
    title: 'Рамочный договор и дополнительное соглашение №1',
    format: 'DOCX',
  },
} as const;

/**
 * Налоговый режим продавца — один источник формулировок для всего сайта.
 *
 * С 01.01.2026 порог освобождения от НДС на УСН снижен с 60 до 20 млн ₽
 * дохода за предыдущий год (ст. 145 НК РФ). Оборот 2025 года порог превысил,
 * поэтому с 2026 года ИП — плательщик НДС и применяет пониженную ставку 5%
 * (п. 8 ст. 164 НК РФ): без вычета входящего налога, зато покупатель на
 * общих ставках предъявленный ему НДС к вычету принимает.
 *
 * Формулировки собраны здесь, а не рассыпаны по страницам: до этого на сайте
 * одновременно жили «в т.ч. НДС 5%» на карточке и «при спецрежиме счёт-фактура
 * не выставляется» в блоге. Клиент читает второе и делает вывод, что вычета
 * не будет, — то есть текст сайта работал против собственного предложения.
 */
export const taxation = {
  /** Ставка НДС в ценах, %. Налог включён в цену, не добавляется сверху. */
  vatPercent: 5,
  /** Дата, с которой применяется режим. */
  since: '2026-01-01',
  /** Короткая строка о цене — там, где о налоге говорится вскользь. */
  priceLine: 'Цены указаны с НДС 5% — налог включён, сверху не добавляется.',
  /** Что получает бухгалтерия. Основной документ — УПД. */
  docsLine:
    'Закрывающие: договор, счёт и УПД (или акт со счётом-фактурой) — '
    + 'сумма НДС выделена отдельной строкой.',
  /** Коммерческий довод: предъявленный налог покупатель принимает к вычету. */
  deductionLine:
    'НДС, который мы предъявляем, покупатель на общей системе принимает к вычету.',
} as const;

/**
 * Параметры калькулятора совокупной стоимости владения подпиской (/tco).
 *
 * Ставки — внешние, они меняются законом, а не нашим решением. Поэтому:
 * 1) живут здесь, а не в компоненте страницы;
 * 2) каждая несёт дату проверки — страница показывает её читателю;
 * 3) при изменении законодательства правится этот блок, и текст страницы
 *    вместе с расчётом обновляется сам.
 *
 * Подтверждены внешним аудитом (юрист + налоговый консультант) 01.09.2026;
 * см. docs/marketing/content-library/facts.md. Порядок поддержки —
 * docs/marketing/tco-calculator-support.md.
 */
export const tcoModel = {
  /** Дата, на которую подтверждены ставки. Выводится на странице. */
  checkedAt: '2026-09-01',
  /** Ставки НДФЛ основной налоговой базы, % (прогрессивная шкала). */
  ndflRates: [13, 15, 18, 20, 22],
  /** Тариф страховых взносов в пределах единой предельной базы, %. */
  insuranceRate: 30,
  /** Тариф взносов с выплат сверх предельной базы, %. */
  insuranceRateAbove: 15.1,
  /** Единая предельная база по взносам на одного работника, ₽. */
  insuranceBase: 2_979_000,
  /** Общая ставка налога на прибыль, %. */
  profitRate: 25,
  /** Значения по умолчанию для сценарных параметров — не нормативы. */
  defaults: {
    /** Банковские, переводные и валютные расходы, % от цены. */
    bankPercent: 15,
    /** Дополнительная компенсация сотруднику, % от его затрат. */
    compensationPercent: 10,
    /** Цена подписки для примера, ₽. */
    price: 100_000,
  },
} as const;

/**
 * Экономика сделки — ставки и базы расчёта прибыли по КП.
 *
 * Решения руководителя 28.08.2026:
 * - налоговая нагрузка 7% считается от ПОЛНОЙ выручки КП (с НДС);
 * - резерв на конвертацию валюты и международный платёж — 10% от суммы
 *   закупки (не от выручки: при высокой наценке иначе завышается расход);
 * - НДС выделяется из цены (5/105), входящего вычета нет — УСН с
 *   пониженной ставкой (см. taxation выше).
 *
 * Ставки живут в конфиге, а не в модуле расчёта: они меняются чаще формул.
 * Природа 7% в коде сознательно не зафиксирована — только имя строки.
 */
export const economics = {
  /** Налоговая нагрузка, % от полной выручки КП. */
  taxPercent: 7,
  /** Название строки в отчётах — без предположений о правовой природе. */
  taxLabel: 'Налоговая нагрузка',
  /** Резерв на конвертацию и международный платёж, % от суммы закупки. */
  fxReservePercent: 10,
  fxReserveLabel: 'Валютные и платёжные расходы',
  /**
   * Продажа/закупка выше этого порога — подозрение, что в карточке товара
   * закупка указана за месяц при годовой продаже. Ошибка уже случалась
   * (поручение руководителя 28.08.2026 — проверять годовые контракты):
   * при обычном коэффициенте наценки ~1.85 такое соотношение честной
   * закупкой не объясняется, а месячной ценой — ровно (≈12×наценка).
   */
  monthlySuspectRatio: 6,
  /** Закупочная цена старше этого срока (дней) — предупреждение о свежести. */
  purchaseStaleDays: 90,
} as const;

/** Главная навигация сайта. */
export const mainNav: { label: string; href: string }[] = [
  { label: 'Каталог', href: '/catalog' },
  { label: 'Как мы работаем', href: '/how-we-work' },
  { label: 'Стоимость', href: '/pricing' },
  { label: 'Документы', href: '/documents' },
  { label: 'База знаний', href: '/blog' },
  { label: 'О нас', href: '/about' },
  { label: 'Контакты', href: '/contacts' },
];

/** Готовые посадочные страницы производителей (для меню «Производители» и /vendors). */
export const vendorLandings: { slug: string; name: string; description: string }[] = [
  // Bespoke-лендинги (отдельные страницы vendors/<slug>.astro)
  { slug: 'zoom', name: 'Zoom', description: 'Видеоконференцсвязь для бизнеса: тарифы Workplace, вебинары, телефония, КП и документы.' },
  // Шаблонные лендинги производителей (креативные индустрии) — из src/data/vendors.ts
  ...VENDORS.map((v) => ({ slug: v.slug, name: v.title || v.vendor, description: v.tagline })),
].sort((a, b) => a.name.localeCompare(b.name, 'ru'));

/** Подвал: дополнительные ссылки. */
export const footerNav: { label: string; href: string }[] = [
  { label: 'AI-сервисы для бизнеса', href: '/catalog/ai' },
  { label: 'Решения под задачу', href: '/solutions' },
  // Разделы сравнений и аналогов до этого не имели входа из навигации:
  // у /alternatives не было ни одной входящей ссылки на сайте вообще, у
  // /compare — только три с главной. В индексе Google не было ни одной
  // страницы обоих разделов (задача IDX-001).
  { label: 'Сравнения сервисов', href: '/compare' },
  { label: 'Аналоги зарубежного ПО', href: '/alternatives' },
  { label: 'Кейсы', href: '/cases' },
  { label: 'FAQ', href: '/faq' },
];

/** ID интеграций аналитики (плейсхолдеры, реальные значения — в .env/прод). */
export const analytics = {
  metrikaId: import.meta.env.PUBLIC_METRIKA_ID || '',
  gaId: import.meta.env.PUBLIC_GA_ID || '',
} as const;
