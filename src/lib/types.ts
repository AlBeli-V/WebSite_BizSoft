...
export interface QuoteItem {
  ...
  /**
   * Производитель позиции (поле `vendor` каталога).
   *
   * Документ КП печатает то же описание, что страница спецификации
   * (`docs/rules/spec-line.md`): юридическое название производителя,
   * название, артикул и договорную фразу. Юрлицо и форма поставки
   * выводятся из марки, поэтому она едет вместе с позицией.
   */
  vendor?: string | null;
  /**
   * В цену позиции включена аренда адреса электронной почты
   * (`docs/rules/email-rent.md`). Документ называет её внутри договорной
   * фразы; сноски «в цене учтена аренда» в бланке нет — она только на сайте.
   */
  email_rent?: boolean;
  /**
