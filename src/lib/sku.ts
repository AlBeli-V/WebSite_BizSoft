 ...
 * источник вида позиции, типа плана, срока и единицы для карточки; по
 * старым артикулам (до перехода) код продолжает работать эвристиками
 ...
 * Старые артикулы вроде `JB-PLG-log-ORG` или `OPENAI-CREDITS-100` сюда не
 * проходят (у них нет сегмента вида и срока), поэтому ложных срабатываний
 * на переходный период нет.
 */
