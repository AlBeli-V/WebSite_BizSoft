...
- **Основные параметры — восемь строк в одном порядке** (`docs/rules/card-params.md`).
  Производитель (знак + `legalName` из реестра вендоров), категория (та же, что
  в плашке под крошками), тип плана, срок плана, расчётная единица (сегмент
  `ЕДИНИЦА` артикула, «Рабочее место» всему каталогу не назначается),
  минимальное количество, НДС `N% (включено в стоимость)` без пояснения,
  артикул. Собирает `cardParams` (`src/lib/card-params.ts`) — не перечень в
  разметке. Строка — подпись и значение на одной оси, в два яруса не
  складывается даже на телефоне. «Тип использования» снят.
