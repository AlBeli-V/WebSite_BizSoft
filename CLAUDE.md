...
- **Bitrix24 — зеркало, воронка в Directus** (`docs/rules/crm-mirror.md`).
  ...
  срок сделки. Обратной синхронизации нет — стадия из Битрикса в отчёт не
  попадает, разметка ведётся в `/admin/leads`. Зеркало не может уронить заявку
  (фоном, исход значением), код вебхука в лог не печатается, без секрета
  контур молча выключен. Секрет `B24_WEBHOOK_URL`, проверка и установка —
  `ops-b24-setup` (`check`, затем `apply`).
