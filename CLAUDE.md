...
- **Артикул — семь сегментов по единой системе** (`docs/rules/sku-system.md`).
  ...
  Каталог переведён 13.09.2026: старые артикулы не используются; до отчёта
  `ops-sku-migrate` «старых sku: 0» слой чтения приводит их по карте
  `data/catalog/sku-map.json`, после — карту и запасные правила снять.
