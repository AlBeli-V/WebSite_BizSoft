...
- **Плашки карточки — вид позиции и одна категория** (`docs/rules/product-markers.md`).
  ...
  `Универсальный продукт` у пополнения, `Дополнение к продукту` у плагина.
  Вторая — одна основная категория; у пополнений и дополнений берётся
  основная категория вендора. Вид считает `cardComposition`, тип плана
  данными не выводится: спорную карточку размечает оператор полем `marker`
  в контенте вендора, а при заведении новой позиции сомнение — это вопрос
